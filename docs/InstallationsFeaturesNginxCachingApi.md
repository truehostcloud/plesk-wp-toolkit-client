# plesk_wp_toolkit_client.InstallationsFeaturesNginxCachingApi

All URIs are relative to *https://example.com/api/modules/wp-toolkit*

Method | HTTP request | Description
------------- | ------------- | -------------
[**update_nginx_caching_status**](InstallationsFeaturesNginxCachingApi.md#update_nginx_caching_status) | **PUT** /v1/installations/{installationId}/features/nginx-caching/status | 


# **update_nginx_caching_status**
> StatusResponse update_nginx_caching_status(installation_id, status_request)



Update nginx caching setting
...
with plesk_wp_toolkit_client.ApiClient(configuration) as api_client:
    ...
    api_instance = plesk_wp_toolkit_client.InstallationsFeaturesNginxCachingApi(api_client)
    installation_id = 1 # int | Installation ID
    status_request = plesk_wp_toolkit_client.StatusRequest() # StatusRequest | 

    try:
        api_response = api_instance.update_nginx_caching_status(installation_id, status_request)
        print("The response of InstallationsFeaturesNginxCachingApi->update_nginx_caching_status:\n")
        pprint(api_response)
    except Exception as e:
        print("Exception when calling InstallationsFeaturesNginxCachingApi->update_nginx_caching_status: %s\n" % e)
```

# plesk_wp_toolkit_client.InstallationsFeaturesIndexingApi

All URIs are relative to *https://example.com/api/modules/wp-toolkit*

Method | HTTP request | Description
------------- | ------------- | -------------
[**update_indexing_status**](InstallationsFeaturesIndexingApi.md#update_indexing_status) | **PUT** /v1/installations/{installationId}/features/indexing/status | 


# **update_indexing_status**
> StatusResponse update_indexing_status(installation_id, status_request)
...
with plesk_wp_toolkit_client.ApiClient(configuration) as api_client:
    ...
    api_instance = plesk_wp_toolkit_client.InstallationsFeaturesIndexingApi(api_client)
    installation_id = 1 # int | Installation ID
    status_request = plesk_wp_toolkit_client.StatusRequest() # StatusRequest | 

    try:
        api_response = api_instance.update_indexing_status(installation_id, status_request)
        print("The response of InstallationsFeaturesIndexingApi->update_indexing_status:\n")
        pprint(api_response)
    except Exception as e:
        print("Exception when calling InstallationsFeaturesIndexingApi->update_indexing_status: %s\n" % e)
```
